...
const {getGovs} = require('../scripts/checkPhase2_gov_mainnet');
...
describe("Governance", async function () {
    ...
    it("member check", async function () {
        const {mainnet, localhost} = await getGovs();

        expect(mainnet.gov.address).to.equal(localhost.gov.address);
    ...
    it("GovImp check", async function () {
        ...
        expect(maxStaking_mainnet).to.equal(minStaking_mainnet);
    ...
    it("envStorage check", async function () {
        ...
        expect(stakingMax_mainnet).to.equal(stakingMin_mainnet);
        expect(stakingMax_localhost).to.equal(ethers.constants.MaxUint256);
        ...
        expect(stakingMinMax_mainnet[0]).to.equal(stakingMinMax_localhost[0]);
        expect(stakingMinMax_mainnet[1]).to.equal(stakingMax_mainnet);
        expect(stakingMinMax_localhost[1]).to.equal(stakingMax_localhost);
        ...
        expect(blockRewardDistributionMethod_mainnet[0]).to.equal(ethers.BigNumber.from(4000));
        expect(blockRewardDistributionMethod_mainnet[1]).to.equal(ethers.BigNumber.from(1000));
        expect(blockRewardDistributionMethod_localhost[0]).to.equal(ethers.BigNumber.from(5000));
        expect(blockRewardDistributionMethod_localhost[1]).to.equal(ethers.BigNumber.from(0));
        expect(blockRewardDistributionMethod_mainnet[2]).to.equal(blockRewardDistributionMethod_localhost[2]);
        expect(blockRewardDistributionMethod_mainnet[3]).to.equal(blockRewardDistributionMethod_localhost[3]);

...
contract GovGatewayImp is GovChecker, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    struct BallotBasic {
        uint256 ballotId;
        //시작 시간
        uint256 startTime;
    ...
    function createVote(uint256 id, address ballotStorage) public view returns (Vote memory) {
        ...
        (uint256 voteId, uint256 ballotId, address voter, uint256 decision, uint256 power, uint256 time) = curBallotStorage.getVote(id);

        return Vote({ voteId: voteId, ballotId: ballotId, voter: voter, decision: decision, power: power, time: time });
    }
    ...
    function getVoteListByBallotId(uint256 ballotId) external view returns (Vote[] memory voteList) {
        ...
        for (uint256 i = 1; i <= voteLength; i++) {
            (, uint256 curBallotId, , , , ) = ballotGateway.getVote(i);
    ...
    function getVoteIdListByBallotId(uint256 ballotId) external view returns (uint256[] memory voteList) {
        ...
        for (uint256 i = 0; i < voteLength; i++) {
            (uint256 curVoteId, uint256 curBallotId, , , , ) = ballotGateway.getVote(i);
            if (curBallotId == ballotId) {
    ...
    */
    ...
    function getBallotVariable(uint256 _id) public view returns (bytes32 envVariableName, uint256 envVariableType, bytes memory envVariableValue) {
        address[] memory ballotAddressList = getBallotStorageAddressList();
        for (uint256 i = 0; i <= ballotAddressList.length; i++) {
            if (isBallot(_id, ballotAddressList[i])) {
    ...
    function getBallotAddress(uint256 _id) public view returns (address newGovernanceAddress) {
        address[] memory ballotAddressList = getBallotStorageAddressList();
        for (uint256 i = 0; i <= ballotAddressList.length; i++) {
            if (isBallot(_id, ballotAddressList[i])) {
    ...
    function getBallotExit(uint256 _id) public view returns (uint256 unlockAmount, uint256 slashing) {
        address[] memory ballotAddressList = getBallotStorageAddressList();
        for (uint256 i = 0; i <= ballotAddressList.length; i++) {
            if (isBallot(_id, ballotAddressList[i])) {

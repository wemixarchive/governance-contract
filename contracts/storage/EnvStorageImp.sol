...
contract EnvStorageImp is AEnvStorage, EnvConstants, UUPSUpgradeable, IEnvStorage {
    ...
    function setBlocksPerByBytes(bytes memory _value) public override onlyGov {
        setUint(BLOCKS_PER_NAME, _toUint(_value));
    }

    function setBallotDurationMinMaxByBytes(bytes memory _value) public override onlyGov {
        (uint256 _min, uint256 _max) = _to2Uint(_value);
        _checkMinMax(_min, _max, "Minimum duration must be smaller and equal than maximum duration");
    ...
    function setStakingMinMaxByBytes(bytes memory _value) public override onlyGov {
        (uint256 _min, uint256 _max) = _to2Uint(_value);
        _checkMinMax(_min, _max, "Minimum staking must be smaller and equal than maximum staking");
    ...
    function setBlockRewardDistributionMethodByBytes(bytes memory _value) public override onlyGov {
        ...
        setUint(BLOCK_REWARD_DISTRIBUTION_ECOSYSTEM_NAME, _ecosystem);
    ...
    function setGasLimitAndBaseFeeByBytes(bytes memory _value) public override onlyGov {
        (uint256 _block_GasLimit, uint256 _baseFeeMaxChangeRate, uint256 _gasTargetPercentage, uint256 _maxBaseFee) = _to4Uint(_value);
    ...
    function checkVariableCondition(bytes32 envKey, bytes memory envVal) external pure override returns (bool) {
        if (envKey == BLOCK_REWARD_DISTRIBUTION_METHOD_NAME) {
            (uint256 _block_producer, uint256 _staking_reward, uint256 _ecofund, uint256 _maintanance) = abi.decode(
                envVal,
    ...
    function _toUint(bytes memory _input) private pure returns (uint256 _output) {
        assembly {
            _output := mload(add(_input, 32))
        }
    }

    function _to2Uint(bytes memory _input) private pure returns (uint256 _output0, uint256 _output1) {
        assembly {
    ...
    function _to4Uint(bytes memory _input) private pure returns (uint256 _output0, uint256 _output1, uint256 _output2, uint256 _output3) {
        assembly {
            _output0 := mload(add(_input, 32))
            _output1 := mload(add(_input, 64))
    ...
    function _checkMinMax(uint256 _min, uint256 _max, string memory _message) private pure returns (uint256, uint256) {
        require(_min <= _max, _message);
        return (_min, _max);
    }

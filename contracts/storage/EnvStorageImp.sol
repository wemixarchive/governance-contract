...
contract EnvStorageImp is AEnvStorage, EnvConstants, UUPSUpgradeable, IEnvStorage {
    ...
    function setBlockRewardDistributionMethod(
        ...
        require((_block_producer + _staking_reward + _ecofund + _maintanance) == DENOMINATOR, "Wrong reward distrubtion ratio");
    ...
    function setBlockRewardDistributionMethodByBytes(bytes memory _value) public override onlyGov {
        (uint256 _block_producer, uint256 _staking_reward, uint256 _ecosystem, uint256 _maintanance) = to4Uint(_value);
        setBlockRewardDistributionMethod(_block_producer, _staking_reward, _ecosystem, _maintanance);
    }

...
async function changeStaking() {
    ...
    for (let i = 0; i < (await gov.getMemberLength()); i++) {
        let ww = fs.readFileSync('./keystore/acct.0'+(i+1));
        govMems.push((await ethers.Wallet.fromEncryptedJson(ww, 'password')).connect(ethers.provider));
    }
    ...
    const StakingImp = await ethers.getContractFactory("StakingImp", govMems[0]);
    const stakingImp = await StakingImp.deploy({gasPrice:'110'+'0'.repeat(9)});
    await stakingImp.deployed();

    const beforeStakingImp = await stakingProxy.implementation();
    let tx = await staking.upgradeStaking(stakingImp.address, {gasPrice:'110'+'0'.repeat(9)});
    await tx.wait();
    ...
    tx = await gov
        ...
        .addProposalToChangeEnv(
            ...
            86400, {gasPrice:'110'+'0'.repeat(9)}
        );
    await tx.wait();

    let ballotLen = await gov.ballotLength();
    for (let i = 0; i < govMems.length; i++) {
        tx = await gov.connect(govMems[i]).vote(ballotLen, true, {gasPrice:'110'+'0'.repeat(9)});
        await tx.wait();

...
async function deployGov(hre, configPath) {
    ...
    deployer = new LedgerSigner(ethers.provider, null, 0);
    ...
    tx = await registry.connect(deployer).setContractDomain(B322S("EnvStorage"), envStorage.address, { gasLimit: txParam.gasLimit, gasPrice: txParam.gasPrice });
    txs.push(tx);
    ...
    tx = await registry.connect(deployer).setContractDomain(B322S("GovernanceContract"), gov.address, { gasLimit: txParam.gasLimit, gasPrice: txParam.gasPrice });
    txs.push(tx);
    ...
    tx = await registry.connect(deployer).setContractDomain(B322S("Ecosystem"), deploy_config.ecosystem, { gasLimit: txParam.gasLimit, gasPrice: txParam.gasPrice });
    txs.push(tx);
    ...
    txFile.txs = txs;
    let receiptFile = {};
    fs.writeFileSync("./ledger/ledger_txs.json", JSON.stringify(txFile, null, 2), "utf-8");
    receipts = [];
    ...
    fs.writeFileSync("./ledger/govDeploy_ledger_receipts.json", JSON.stringify(receiptFile, null, 2), "utf-8");
    ...
    fs.writeFileSync("./ledger/gov_ledger_contracts.json", JSON.stringify(contractData), "utf-8", function (e) {
        if (e) {
            console.log(e);
        } else {
